...
dw_bar<-function(tab, type="split", mirror=T, colors=c("#003f5c", "#fa8c00")){
  cats<-colnames(tab[,-1])
  ...
  dw_edit_chart(chart_id = id,  type=plot, language="ru-RU", visualize=list("value-label-format"="0.0","background"=F, "show-color-key"=color_key, "mirror-bars"=mirror, "custom-colors"=cat_col))
